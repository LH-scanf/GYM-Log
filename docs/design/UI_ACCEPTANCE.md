...
- 日期 / 开始时间在 **390 × 844 下保持两列**，不得因为窄屏退化成单列堆叠
- 两个字段是「小标签在上、紧凑值在下」的短字段，不各占一整行
- 值由应用紧凑渲染（`9月16日` / `13:44`），但控件本体必须仍然是原生
  `input[type=date]` / `input[type=time]`，点击仍由系统原生选择器接管
- 字段可点区域 ≥ 44px 高

MUST NOT:

- 在 390 × 844 下把两个短字段拆成两行大输入框
- 为了塞进两列而换成自定义（非原生）日期 / 时间选择器
- 用「窄屏」当理由给短字段加单列降级

锁定：`tests/e2e/new-workout-fields.spec.ts`。
390 是主设计宽度，不是「需要降级成单列」的极窄屏。
...
[icon] 今年训练次数    [icon] 今年训练时长
       12 次                  8h30min
[icon] 本月训练次数    [icon] 本月训练时长
       3 次                   2h10min
...
MUST:
- 页头为左对齐 28px 大标题 + 右侧年份胶囊（`.page-header`，一级页不再叠加 `.top-bar`）
- 指标卡 2×2，390px 下不降级成单列
- 指标卡四色图标成对取用 `--color-metric-*`（图标色 + 图标底）
- 趋势卡默认「训练场次最多的动作」，点胶囊弹出 Sheet 换动作
- 普通负重动作提供「估算 1RM / 最高重量」两个口径互切
- Heatmap 7 rows
- Week columns 横向
- Cell 视觉为 square
- 不在每个 cell 内显示日期数字
- Card 内必要时横向滚动
- 月份刻度不得折行（标签横跨 4 周，见 `.heatmap-months__label`）
...
- Heatmap 占几十屏
- Body 因 Heatmap 横向滚动
- 出现「从本地训练记录实时计算」这类实现说明
- 出现「有训练的日期已高亮。点击日期查看当日训练。」这类操作说明
- 指标卡标签里再写年份（`2026 年训练次数`），年份由页头胶囊承担
...
- 大段说明文案 + 巨型 CTA 像营销页
- 默认展示完整导入摘要占位
- 在页头下方再叠一个 `本地数据与备份` 之类的 eyebrow 小标签
